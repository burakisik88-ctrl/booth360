...
function bantKat(metin,spec,yon){
  ...
  var y=Math.max(0,Math.min(400,+spec.by||92));
...
function chain(spec){
  ...
  var kp=kadraj(spec.k);if(kp)parts.push(kp);
  ...
  if(o.bant){
    if(spec.bu)parts=parts.concat(bantKat(spec.bu,spec,"ust"));
    if(spec.ba)parts=parts.concat(bantKat(spec.ba,spec,"alt"));
  }
...
function hamChain(spec){
  var parts=[],kp=kadraj(spec.k),o=flagsOf(spec);
  if(kp)parts.push(kp);
  if(o.logo&&spec.l){
...
function encode(spec){
  ...
  ["c","p","v","t","d","f","r","l","lp","lw","m","x","fp","k","bu","ba","bc","bs","bf","by"].forEach(function(k){if(spec[k]!==undefined&&spec[k]!==""&&spec[k]!==null)q.set(k,String(spec[k]))});
...
function decode(str){
  ...
  ["c","p","v","t","d","f","r","l","lp","lw","m","x","fp","k","bu","ba","bc","bs","bf","by"].forEach(function(k){if(q.has(k))spec[k]=q.get(k)});
  var o={},s=q.get("o")||"";Object.keys(FK).forEach(function(k){o[k]=s.indexOf(FK[k])>=0?1:0});
  o.tol=+(q.get("tl")||25);spec.o=o;
  return spec;
}
return {TEMPLATES:TEMPLATES,byId:byId,plan:plan,chain:chain,url:url,bar:bar,encode:encode,decode:decode,facePick:facePick,DEFO:DEFO,lid:lid,hamIslem:hamIslem,kadraj:kadraj,bantKat:bantKat,bantVar:bantVar,txtEnc:txtEnc};
})();
